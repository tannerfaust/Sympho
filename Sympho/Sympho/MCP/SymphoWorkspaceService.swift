...
final class SymphoWorkspaceService {
    ...
    private func insert(kind: WorkspaceEntityKind, fields: [String: WorkspaceValue]) throws -> WorkspaceEntityDTO {
        ...
        case .track:
            ...
            let track = Track(id: id, title: try fields.requiredString("title", max: 500), desc: fields["description"]?.string ?? "", sortIndex: fields["sort_index"]?.int ?? 0, domain: parent)
            track.emoji = glyphEmoji(fields); track.iconName = fields["icon_name"]?.string ?? ""
        ...
        case .module:
            ...
            let module = Module(id: id, title: try fields.requiredString("title", max: 500), desc: fields["description"]?.string ?? "", sortIndex: fields["sort_index"]?.int ?? 0, track: track, domain: domain)
            module.emoji = glyphEmoji(fields); module.iconName = fields["icon_name"]?.string ?? ""
            context.insert(module)
        case .project:
            let project = Project(id: id, title: try fields.requiredString("title", max: 500), desc: fields["description"]?.string ?? "", status: ProjectStatus(rawValue: fields["status"]?.string ?? "") ?? .backlog, isPinned: fields["pinned"]?.bool ?? false, domain: try optional(Domain.self, id: try fields.uuid("domain_id")), track: try optional(Track.self, id: try fields.uuid("track_id")))
            project.emoji = glyphEmoji(fields); project.iconName = fields["icon_name"]?.string ?? ""
            context.insert(project)
        case .node:
            let node = Node(id: id, title: try fields.requiredString("title", max: 500), desc: fields["description"]?.string ?? "", sortIndex: fields["sort_index"]?.int ?? 0, status: NodeStatus(rawValue: fields["status"]?.string ?? "") ?? .backlog, priority: NodePriority(rawValue: fields["priority"]?.string ?? "") ?? .normal, isOrphan: fields["orphan"]?.bool ?? false, captureIntent: CaptureIntent(rawValue: fields["capture_intent"]?.string ?? "") ?? .learningNode, module: try optional(Module.self, id: try fields.uuid("module_id")), project: try optional(Project.self, id: try fields.uuid("project_id")))
            node.emoji = glyphEmoji(fields); node.iconName = fields["icon_name"]?.string ?? ""
            context.insert(node)
        case .resource:
    ...
    private func apply(kind: WorkspaceEntityKind, id: UUID, fields: [String: WorkspaceValue]) throws {
        ...
        case .track: let v=try require(Track.self,id:id); if let x=fields["title"]?.string{v.title=x}; if let x=fields["description"]?.string{v.desc=x}; if let x=fields["sort_index"]?.int{v.sortIndex=x}; applyGlyph(fields,emoji:{v.emoji=$0},icon:{v.iconName=$0}); if fields["domain_id"] != nil {v.domain=try optional(Domain.self,id:try fields.uuid("domain_id"))}; touch(v)
        case .module: let v=try require(Module.self,id:id); if let x=fields["title"]?.string{v.title=x}; if let x=fields["description"]?.string{v.desc=x}; if let x=fields["sort_index"]?.int{v.sortIndex=x}; applyGlyph(fields,emoji:{v.emoji=$0},icon:{v.iconName=$0}); if fields["track_id"] != nil {v.track=try optional(Track.self,id:try fields.uuid("track_id"));v.domain=nil}; if fields["domain_id"] != nil {v.domain=try optional(Domain.self,id:try fields.uuid("domain_id"));v.track=nil}; guard (v.track != nil) != (v.domain != nil) else {throw WorkspaceServiceError.validation("Module must have exactly one parent")}; touch(v)
        case .project: let v=try require(Project.self,id:id); if let x=fields["title"]?.string{v.title=x}; if let x=fields["description"]?.string{v.desc=x}; if let x=fields["status"]?.string, let e=ProjectStatus(rawValue:x){v.status=e}; if let x=fields["pinned"]?.bool{v.isPinned=x}; applyGlyph(fields,emoji:{v.emoji=$0},icon:{v.iconName=$0}); if fields["domain_id"] != nil {v.domain=try optional(Domain.self,id:try fields.uuid("domain_id"))}; if fields["track_id"] != nil {v.track=try optional(Track.self,id:try fields.uuid("track_id"))}; touch(v)
        case .node: let v=try require(Node.self,id:id); if let x=fields["title"]?.string{v.title=x}; if let x=fields["description"]?.string{v.desc=x}; if let x=fields["status"]?.string,let e=NodeStatus(rawValue:x){v.status=e}; if let x=fields["priority"]?.string,let e=NodePriority(rawValue:x){v.priority=e}; if let x=fields["pinned"]?.bool{v.isPinned=x}; if let x=fields["sort_index"]?.int{v.sortIndex=x}; applyGlyph(fields,emoji:{v.emoji=$0},icon:{v.iconName=$0}); if fields["module_id"] != nil {v.module=try optional(Module.self,id:try fields.uuid("module_id"))}; if fields["project_id"] != nil {v.project=try optional(Project.self,id:try fields.uuid("project_id"))}; touch(v)
    ...
    private func dto(_ v: Track)->WorkspaceEntityDTO { entity(v.id,.track,v.title,["description":.string(v.desc),"sort_index":.int(v.sortIndex),"emoji":.string(v.emoji),"icon_name":.string(v.iconName)],["domain":ids(v.domain)],v.isDeletedLocally,v.createdAt,v.updatedAt) }
    private func dto(_ v: Module)->WorkspaceEntityDTO { entity(v.id,.module,v.title,["description":.string(v.desc),"sort_index":.int(v.sortIndex),"emoji":.string(v.emoji),"icon_name":.string(v.iconName)],["track":ids(v.track),"domain":ids(v.domain)],v.isDeletedLocally,v.createdAt,v.updatedAt) }
    private func dto(_ v: Project)->WorkspaceEntityDTO { entity(v.id,.project,v.title,["description":.string(v.desc),"status":.string(v.statusValue),"pinned":.bool(v.isPinned),"emoji":.string(v.emoji),"icon_name":.string(v.iconName)],["domain":ids(v.domain),"track":ids(v.track),"nodes":v.nodes.map(\.id),"resources":v.resources.map(\.id)],v.isDeletedLocally,v.createdAt,v.updatedAt) }
    private func dto(_ v: Node)->WorkspaceEntityDTO { entity(v.id,.node,v.title,["description":.string(v.desc),"status":.string(v.statusValue),"priority":.string(v.priorityValue),"sort_index":.int(v.sortIndex),"pinned":.bool(v.isPinned),"emoji":.string(v.emoji),"icon_name":.string(v.iconName)],["module":ids(v.module),"project":ids(v.project),"resources":v.resources.map(\.id)],v.isDeletedLocally,v.createdAt,v.updatedAt) }
    ...
    /// First emoji grapheme of the `emoji` field, whitespace stripped.
    private func glyphEmoji(_ f:[String:WorkspaceValue])->String { (f["emoji"]?.string?.trimmingCharacters(in:.whitespacesAndNewlines)).flatMap { $0.first.map(String.init) } ?? "" }
    /// Applies `emoji` / `icon_name` updates if present; setting one clears the other.
    private func applyGlyph(_ f:[String:WorkspaceValue],emoji setEmoji:(String)->Void,icon setIcon:(String)->Void) {
        if let raw=f["emoji"]?.string { let e=raw.trimmingCharacters(in:.whitespacesAndNewlines).first.map(String.init) ?? ""; setEmoji(e); if !e.isEmpty { setIcon("") } }
        if let icon=f["icon_name"]?.string { setIcon(icon); if !icon.isEmpty { setEmoji("") } }
    }
